...
if global.morto {
    ...
    draw_text(_center_x, _center_y - 110, "Game Over");

    // Configurações de desenho para a pontuação
    draw_set_font(fnt_pontuacao);
    draw_set_font(fnt_pontuacao);
	if global.pontuacao_atual >= global.melhor_pontuacao{
		draw_text(_center_x, _center_y - 80, "Novo recorde: " + string(global.pontuacao_atual));
	}else{
		draw_text(_center_x, _center_y - 80, "Pontuação atual: " + string(global.pontuacao_atual));
		draw_text(_center_x, _center_y - 60, "Melhor pontuação: " + string(global.melhor_pontuacao));
	}

    // Cria botões de retorno ao menu inicial e reinício do jogo
    create_button(_center_x - 90, _center_y, spr_home, button_close_action);
    create_button(_center_x + 90, _center_y, spr_restart, button_play_action);
}
